...
Concerning our visualization blog posts. Currently, this blog covers the basic of Matplotlib. The first blog post, (An Introduction to Visualizations in Python),  show you how to plot a single factor, even if that factor exists in multiple datasets. We give in-depth coverage of basic plots, and in our Matplotlib introduction blog post , we cover histograms, boxplots, and density functions. If you have read that post, then you understand how in depth we go into every function so that way you not only can use the plotting functions, but can customize them to look however you wihs. 
